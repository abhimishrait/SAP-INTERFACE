// Postman-like API Tester — request builder + simulated response

function ApiTester({ selectedModule, setSelectedModule, theme }) {
  const moduleId = selectedModule || 'bp-master';
  const mod = MODULE_BY_ID[moduleId];

  // Imported-collection state — synced to localStorage
  const [imported, setImported] = React.useState(() => getImportedCollections());
  const [importOpen, setImportOpen] = React.useState(false);
  const [activeImported, setActiveImported] = React.useState(null); // {colId, reqIdx} | null

  // method state — preselect first allowed method
  const [method, setMethod] = React.useState(mod.methods[0]);
  const [recordId, setRecordId] = React.useState('1');
  const [tab, setTab] = React.useState('body'); // params | auth | headers | body
  const [body, setBody] = React.useState(SAMPLE_PAYLOADS[moduleId]?.request || '{}');
  ...
  // When module changes, reset method + body
  React.useEffect(() => {
    setMethod(mod.methods[0]);
    setBody(SAMPLE_PAYLOADS[moduleId]?.request || '{}');
    setResponse(null);
  }, [moduleId]);

  const isPutWithId = method === 'PUT' && mod.methods.includes('POST'); // §3.x — both PUT-only modules don't take {id}
  const fullPath = isPutWithId ? `${mod.path}${recordId}/` : mod.path;
  const fullUrl = `${BASE_URL}${fullPath}`;

  // When user picks an imported request, copy it into the editor
  const openImported = (colId, reqIdx) => {
    const col = imported.find(c => c.id === colId);
    const r = col?.requests[reqIdx];
    if (!r) return;
    setActiveImported({ colId, reqIdx });
    setSelectedModule(null); // disconnect from spec module
    setMethod(r.method);
    setBody(r.body || '');
  };
  const onImportedReceived = (col) => {
    setImported(getImportedCollections());
  };
  const removeCollection = (colId) => {
  ...
  const onSend = () => {
    ...
    setTimeout(() => {
      ...
      let result;
      // Naive validation: empty body / invalid JSON / missing required
      let parsed = null;
      ...
      // Check required fields against mapping spec
      const required = (MAPPINGS_BY_MODULE[moduleId] || []).filter(f => f.required);
      const missing = [];
      const flat = JSON.stringify(parsed || {});
      for (const f of required) {
        const fieldKey = f.sap.split('[')[0].split('.')[0];
      ...
      // success
      const sample = SAMPLE_PAYLOADS[moduleId]?.response;
        ...
        <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--line)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'stretch', gap: 8 }}>
            ...
            <button onClick={onSend} disabled={sending} className="btn primary" style={{ padding: '0 22px', height: 36, fontSize: 13, fontWeight: 700 }}>
              {sending ? <><span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: '50%', border: '2px solid currentColor', borderTopColor: 'transparent', animation: 'spin 0.7s linear infinite' }} /> Sending</> : <><Icons.send /> Send</>}
            </button>
...
// Imported collection block (collapsible)
...
function TesterCollection({ moduleId, setSelectedModule, method, setMethod, imported, activeImported, openImported, onOpenImportModal, removeCollection }) {
  ...
  const filterMod = (m) => !query || m.label.toLowerCase().includes(query.toLowerCase()) || m.path.includes(query);
  ...
  return (
    <aside style={{
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '6px 0' }}>
        ...
        {groups.map(g => {
          const mods = MODULES.filter(m => m.kind === g.id).filter(filterMod);
          ...
          return (
            <div key={g.id} style={{ paddingLeft: 14 }}>
              <div
                ...
                style={{
                  display: 'flex', alignItems: 'center', gap: 8,
                  padding: '5px 14px 5px 4px', cursor: 'pointer',
                }}>
...
// ── Tabs ───────────────────────────────────────────────────────────────────
function ParamsTab({ moduleId, method }) {
  return (
    <div>
...
function HeadersTab({ headers, setHeaders }) {
  const toggle = (i) => setHeaders(hs => hs.map((h, j) => j===i ? { ...h, enabled: !h.enabled } : h));
  const update = (i, k, v) => setHeaders(hs => hs.map((h, j) => j===i ? { ...h, [k]: v } : h));
...
function BodyTab({ body, setBody, moduleId }) {
  const [mode, setMode] = React.useState('raw'); // raw | form
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'flex', gap: 14, marginBottom: 10, alignItems: 'center' }}>
        ...
        <button className="btn ghost" style={{ fontSize: 11 }} onClick={() => setBody(SAMPLE_PAYLOADS[moduleId]?.request || '{}')}>
      ...
      <div style={{ flex: 1, minHeight: 180, border: '1px solid var(--line)', borderRadius: 8, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <textarea
          ...
          style={{
            ...
            color: 'var(--ink-1)', fontFamily: 'JetBrains Mono, monospace', fontSize: 12,
...
function ResponseHeaders({ response, moduleId, method }) {
  const codeText = RESPONSE_CODES.find(c => c.code === response.status)?.status || 'OK';
  const mod = MODULE_BY_ID[moduleId];
  const hdrs = [
...
function ResponseTests({ response, moduleId }) {
  const mod = MODULE_BY_ID[moduleId];
  const isOk = response.status < 400;
  const tests = [
...
function ResponseLogs({ response, moduleId, method }) {
...
Object.assign(window, { ApiTester });
