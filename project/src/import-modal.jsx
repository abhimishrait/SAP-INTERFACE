// Import Collection Modal — paste JSON or upload Postman/OpenAPI

const IMPORTED_COLLECTIONS_KEY = 'salesport_imported_collections_v1';

function getImportedCollections() {
  try {
    const raw = localStorage.getItem(IMPORTED_COLLECTIONS_KEY);
    return raw ? JSON.parse(raw) : [];
  } catch { return []; }
}
function saveImportedCollections(cs) {
  localStorage.setItem(IMPORTED_COLLECTIONS_KEY, JSON.stringify(cs));
}

// Convert various formats to our internal { id, name, source, requests: [{ name, method, url, body, headers }] }
    ...
    for (const it of items) {
      if (it.item) {
        ...
        out.push({
          ...
          url: url || '',
          body: r.body?.raw || '',
          headers: (r.header || []).filter(h => !h.disabled).map(h => ({ key: h.key, value: h.value })),
        });
      }
    }
    return out;
...
function parseOpenAPI(json) {
  ...
  for (const [path, ops] of Object.entries(json.paths || {})) {
    for (const m of ['get','post','put','patch','delete']) {
      if (ops[m]) {
        requests.push({
          name: ops[m].summary || ops[m].operationId || `${m.toUpperCase()} ${path}`,
          group: ops[m].tags?.[0] || 'default',
...
function tryParseCollection(text) {
  let json;
  ...
  if (json.collection && json.collection.item) return parsePostman(json.collection); // wrapped
  throw new Error('Unknown collection format. Supported: Postman v2.1, OpenAPI 3.x, Swagger 2.0.');
}

// Sample seed for the "try it" button
...
function ImportModal({ open, onClose, onImported }) {
  ...
  const [tab, setTab] = React.useState('paste'); // paste | upload | url
  ...
  const onFileChosen = async (file) => {
    ...
    reader.onload = (e) => { setText(e.target.result); onParse(e.target.result); };
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <textarea
                  ...
                  style={{
                    ...
                    color: 'var(--ink-1)', fontFamily: 'JetBrains Mono, monospace', fontSize: 11.5, lineHeight: 1.5, outline: 'none',
                  }}
                />
...
Object.assign(window, { ImportModal, getImportedCollections, saveImportedCollections });
