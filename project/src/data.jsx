...
// Base URL & auth from spec
const BASE_URL = 'http://dms.salesport.in';
const AUTH_USER = 'SujalFoods';
const AUTH_HEADER = 'Basic U3VqYWxGb29kczpTdWphbEZvb2RzQDEyMw==';

// ── 16 MODULES from spec section 3 ──────────────────────────────────────────
const MODULES = [
  {
...
// SAP B1 source → SalesPort DMS target
// Each: { sap, sapType, sapDesc, dms, dmsType, dmsDesc, xform, status, confidence, required }
const MAPPINGS_BY_MODULE = {
  'bp-master': [
...
// ── STATUS MAPPING from spec section 4.1 ──────────────────────────────────
...
// ── TRANSACTIONS (real endpoints) ─────────────────────────────────────────
const NOW = new Date('2026-05-22T11:42:18+05:45'); // Nepal Time
function pad(n, w=2){ return String(n).padStart(w,'0'); }
function genTxId(seed){
...
const TX_DEFS = [
  // [moduleId, method, status, distributor]
...
const TRANSACTIONS = TX_DEFS.map(([moduleId, method, status, distributor], i) => {
...
function iso(date){
  const d = date;
  return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}.${pad(d.getMilliseconds(),3)}+05:45`;
}

// ── SAMPLE PAYLOADS (per module) ───────────────────────────────────────────
...
function getResponseHeaders(tx) {
...
Object.assign(window, {
  Icons, MODULES, MODULE_BY_ID, MAPPINGS_BY_MODULE,
  TRANSACTIONS, QUEUE_JOBS, QUEUE_RECENT,
  DB_TABLES, CONNECTORS, NOW, relTime, iso,
  STATUS_MAPPING, RESPONSE_CODES, ERROR_SCENARIOS,
  SAMPLE_PAYLOADS, getHeadersForTx, getResponseHeaders,
  BASE_URL, AUTH_USER, AUTH_HEADER,
});
