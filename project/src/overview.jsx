// Overview view: real SAP B1 → SalesPort modules

function Overview({ flowAnim, density }) {
  ...
  return (
    <div style={{ padding: density==='compact' ? 18 : 24, overflow: 'auto', height: '100%' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 20 }}>
        <Stat label="Requests · 24h"   value="42,318"   trend="+8.4%" sub="vs prior 24h"
              icon={<Sparkline seed={2} stroke="var(--teal)" w={88} h={28} />} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 14, marginBottom: 20 }}>
        <div className="card">
          ...
          <div className="body">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, fontSize: 10, color: 'var(--ink-3)', fontFamily: 'JetBrains Mono, monospace' }}>
...
function PipelineRow({ name, rate, reqs, status }) {
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--ink-2)', fontFamily: 'JetBrains Mono, monospace' }}>{reqs}</span>
        </div>
        <div className="prog">
...
// Flow diagram: real SAP B1 module flow → mapper → DMS persistence
function FlowDiagram({ animate, tick }) {
  // Pick most visible modules for the diagram
  const sapNodes = [
...
function ColumnHeader({ label, tag, color, align }) {
  return (
...
function FlowNode({ label, rate, icon, side, active }) {
  const c = side==='sap' ? 'var(--teal)' : 'var(--orange)';
  return (
    <div style={{
      padding: '10px 12px',
      background: active ? (side==='sap' ? 'var(--teal-bg)' : 'var(--orange-bg)') : 'var(--bg-2)',
      ...
      <div style={{
        ...
        background: 'var(--bg-3)', border: `1px solid ${c}`,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontFamily: 'JetBrains Mono, monospace', fontSize: 10, fontWeight: 700, color: c,
      }}>{icon}</div>
      <div style={{ flex: 1, minWidth: 0 }}>
...
function TransformEngine({ animate }) {
  return (
    <div style={{
      width: 200, padding: '18px 16px',
      background: 'var(--bg-2)', border: '1px solid var(--line-warm)',
      borderRadius: 12, position: 'relative', zIndex: 3,
      boxShadow: '0 0 0 4px rgba(245,155,61,0.04), 0 12px 32px rgba(0,0,0,0.4)',
    }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <EngineStep label="Verify Basic auth" />
        <EngineStep label="Validate schema" />
        <EngineStep label="Map fields" highlight />
        <EngineStep label="FK lookups + dedupe" />
        <EngineStep label="Idempotency check" />
        <EngineStep label="Persist + audit" />
      </div>
      <div style={{ marginTop: 12, display: 'flex', justifyContent: 'space-between', fontSize: 10, fontFamily: 'JetBrains Mono, monospace', color: 'var(--ink-2)' }}>
        <span>spec v1.2</span>
...
function EngineStep({ label, highlight }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: highlight ? 'var(--ink-0)' : 'var(--ink-1)' }}>
      <span style={{ width: 5, height: 5, borderRadius: '50%', background: highlight ? 'var(--orange)' : 'var(--teal-dim)' }} />
      {label}
    </div>
  );
}

Object.assign(window, { Overview, Header });
