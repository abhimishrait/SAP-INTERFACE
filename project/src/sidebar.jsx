function Sidebar({ view, setView, user, onLogout }) {
  const items = [
    { id: 'overview', label: 'Overview',      ico: Icons.overview, badge: null },
  ...
  return (
    <aside style={{
      ...
      {/* env switch */}
      <div style={{
        background: 'var(--bg-2)', borderRadius: 8, padding: 10, marginBottom: 18,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        {items.map(it => (
          <div key={it.id} className={`nav-item ${view===it.id ? 'active' : ''}`} onClick={() => setView(it.id)}>
            ...
            {it.badge && (
              <span style={{
                fontSize: 10, fontFamily: it.badge==='live' ? 'inherit' : 'JetBrains Mono, monospace',
                fontWeight: 600,
    ...
    <div style={{ position: 'relative' }}>
      {open && (
        <div style={{
          ...
          padding: 4, boxShadow: '0 12px 32px rgba(0,0,0,0.4)', zIndex: 10,
        }}>
          ...
          <MenuItem label="Sign out"         icon={<LogoutIcon />}   danger onClick={() => { setOpen(false); onLogout && onLogout(); }} />
        </div>
      )}
      <div
        onClick={() => setOpen(o => !o)}
        ...
        style={{
          ...
          background: 'var(--bg-2)', border: `1px solid ${open ? 'var(--line-strong)' : 'var(--line)'}`,
...
function MenuItem({ label, icon, onClick, danger }) {
  return (
    <div
      ...
      onMouseEnter={e => { e.currentTarget.style.background = danger ? 'var(--red-bg)' : 'var(--bg-3)'; }}
...
function SystemCard({ label, tag, color, detail }) {
  return (
    <div style={{
      padding: 10, borderRadius: 8,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingLeft: 12 }}>
        ...
        <span style={{ fontSize: 10, color: 'var(--ink-2)', fontFamily: 'JetBrains Mono, monospace' }}>{detail}</span>
      </div>
    </div>
  );
}
