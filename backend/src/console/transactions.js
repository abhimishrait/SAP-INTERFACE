...
router.get('/', async (req, res, next) => {
  try {
    ...
    const rows = await query(
      `SELECT id, correlation_id AS tx_id, module_id, method, path, resource_id,
    ...
    const total = await one(
      `SELECT COUNT(*) AS c FROM integration_transactions ${wsql}`, params
    );
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const row = await one(
      `SELECT * FROM integration_transactions WHERE id = ? LIMIT 1`, [id]
    );
