...
export function useTweaks(defaults: TweakValues): [TweakValues, (key: string, val: unknown) => void] {
  ...
  const setTweak = React.useCallback((keyOrEdits: string | Partial<TweakValues>, val?: unknown) => {
...
export function TweakRadio({ label, value, options, onChange }: {
  ...
  const lookup = ({ 2: 16, 3: 10 } as Record<number, number>)[options.length];
  const fitsAsSegments = lookup !== undefined ? maxLen <= lookup : false;

  if (!fitsAsSegments) {
    return <TweakSelect label={label} value={value} options={options.map(o => ({ value: o, label: o }))} onChange={onChange} />;
  }
...
export function TweakButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <div className="twk-row twk-row-h">
      <div className="twk-lbl"><span>{label}</span></div>
      <button type="button" className="twk-btn secondary" onClick={onClick}>{label}</button>
    </div>
  );
}
