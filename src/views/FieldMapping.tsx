...
export default function FieldMapping({ selectedModule, setSelectedModule, density }: {
  ...
  const counts = {
    ...
    mapped: allMappings.filter(f => f.status === 'mapped').length,
  };
    ...
    <div style={{ padding: 24, height: '100%', overflow: 'auto' }}>
      ...
      <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 8, marginBottom: 12, borderBottom: '1px solid var(--line)' }}>
        {MODULES.map(m => (
          <button key={m.id} onClick={() => setSelectedModule(m.id)} style={{
            ...
            {m.label}
            <span className="mono" style={{ fontSize: 10, color: moduleId === m.id ? 'var(--orange)' : 'var(--ink-3)', background: moduleId === m.id ? 'var(--orange-bg)' : 'var(--bg-3)', padding: '1px 5px', borderRadius: 3 }}>
              {(MAPPINGS_BY_MODULE[m.id] || []).length}
            </span>
          </button>
        ))}
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', marginBottom: 14, background: 'var(--bg-1)', border: '1px solid var(--line)', borderRadius: 10 }}>
        <div>
        ...
        <div className="meta-row">
          <div><label>Methods</label><div style={{ display: 'flex', gap: 4, marginTop: 2 }}>{mod.methods.map(m => <Method key={m} m={m} />)}</div></div>
          <div><label>Endpoint</label><span className="val" style={{ color: 'var(--orange)' }}>{mod.path}</span></div>
          <div><label>Rate</label><span className="val">~{mod.rps}/s</span></div>
          <div><label>Error rate</label><span className="val" style={{ color: mod.errRate > 1 ? 'var(--amber)' : 'var(--teal)' }}>{mod.errRate.toFixed(2)}%</span></div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        {[
          ...
          { id: 'mapped',   label: 'Mapped',   count: counts.mapped,   color: 'var(--teal)' },
        ...
        <div className="card">
          <div className="hd">
            ...
            <div style={{ display: 'flex', gap: 14, fontSize: 11, color: 'var(--ink-2)' }}>
              {[['var(--teal)', 'mapped'], ['var(--amber)', 'review'], ['var(--orange)', 'required']].map(([c, l]) => (
                <span key={l} style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ width: 8, height: 8, background: c, borderRadius: 2 }} /> {l}</span>
              ))}
            </div>
          </div>
          <MappingCanvas mappings={filtered} selectedIdx={selectedIdx} setSelectedIdx={setSelectedIdx} />
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        <div className="card">
          <div className="hd"><h3>Business rules & validation</h3><span className="mono" style={{ fontSize: 11, color: 'var(--ink-3)' }}>{mod.rules.length}</span></div>
          <div className="body">
        ...
        <div className="card">
          <div className="hd"><h3>Sample request → response</h3><span className="mono" style={{ fontSize: 11, color: 'var(--ink-3)' }}>from spec v1.2</span></div>
...
function SamplePayloadView({ moduleId }: { moduleId: string }) {
...
function truncateFn(xform: string) {
  if (xform === 'direct') return 'direct';
  if (xform === 'lookup' || xform.startsWith('lookup')) return 'lookup';
...
function MappingCanvas({ mappings, selectedIdx, setSelectedIdx }: {
  ...
  const rowH = 36, topPad = 50, sideW = 320;
  const totalH = Math.max(topPad + mappings.length * rowH + 20, 400);
  const statusColor = (s: string) => s === 'mapped' ? 'var(--teal)' : s === 'review' ? 'var(--amber)' : 'var(--red)';
    ...
    <div style={{ position: 'relative', display: 'grid', gridTemplateColumns: `${sideW}px 1fr ${sideW}px`, gap: 0, padding: '18px 0' }}>
      <div style={{ paddingLeft: 18 }}>
        ...
        <div style={{ marginTop: 16 }}>
          {mappings.map((f, i) => (
            <FieldRow key={f.sap + i} field={f.sap} type={f.sapType} desc={f.sapDesc} required={f.required} side="left" active={i === selectedIdx} onClick={() => setSelectedIdx(i)} statusColor={statusColor(f.status)} />
          ))}
        </div>
      </div>
      <div style={{ position: 'relative' }}>
        <svg width="100%" height={totalH} style={{ display: 'block', position: 'absolute', inset: 0 }} preserveAspectRatio="none" viewBox={`0 0 200 ${totalH}`}>
          {mappings.map((f, i) => {
            ...
            return (
              <g key={i}>
                <path d={`M 0 ${y} C 60 ${y}, 140 ${y}, 200 ${y}`} stroke={c} strokeWidth={isSel ? 2 : 1.2} fill="none" opacity={isSel ? 1 : 0.45} />
                <g transform={`translate(72 ${y - 9})`}>
                  ...
                  <text x="28" y="12" fontFamily="monospace" fontSize="9" fill={c} textAnchor="middle" fontWeight="600" opacity={isSel ? 1 : 0.85}>
      ...
      <div style={{ paddingRight: 18 }}>
        ...
        <div style={{ marginTop: 16 }}>
          {mappings.map((f, i) => (
            <FieldRow key={f.dms + i} field={f.dms} type={f.dmsType} desc={f.dmsDesc} side="right" active={i === selectedIdx} onClick={() => setSelectedIdx(i)} statusColor={statusColor(f.status)} />
          ))}
        </div>
      </div>
    </div>
...
function ColHeader2({ label, sub, color, align }: { label: string; sub: string; color: string; align: string }) {
  return (
...
function FieldRow({ field, type, desc, required, side, active, onClick, statusColor }: {
  ...
  side: string; active: boolean; onClick: () => void; statusColor: string;
}) {
  return (
    <div onClick={onClick} className="clickable" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 10px', borderRadius: 6, background: active ? 'var(--bg-3)' : 'transparent', border: `1px solid ${active ? statusColor : 'transparent'}`, marginBottom: 4, height: 32, flexDirection: side === 'right' ? 'row-reverse' : 'row', textAlign: side === 'right' ? 'right' : 'left' }}>
...
function FieldInspector({ field, moduleId }: { field: FieldMappingType; moduleId: string }) {
  ...
  return (
    <div className="card" style={{ position: 'sticky', top: 0, height: 'fit-content' }}>
      <div className="hd"><h3>Field inspector</h3><Chip kind={statusKind} dot>{field.status}</Chip></div>
          ...
          {field.confidence > 0 && (
            <div style={{ marginTop: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10.5, color: 'var(--ink-2)', marginBottom: 4 }}>
                <span>Confidence</span><span className="mono">{field.confidence}%</span>
              </div>
