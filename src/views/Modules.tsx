...
export default function Modules({ setSelectedModule, setView }: {
  ...
  const open = (id: string) => { setSelectedModule(id); setView('mapping'); };
            ...
            <tbody>
              {STATUS_MAPPING.map(s => (
                <tr key={s.input}>
                  ...
                  <td>
                    <Chip kind={['Active', 'OPEN'].includes(s.interpreted) ? 'ok' : ['Inactive', 'CANCELLED', 'CLOSED'].includes(s.interpreted) ? 'muted' : 'info'} dot>
                      {s.interpreted}
                    </Chip>
                  </td>
        ...
        <div className="card">
          ...
          <div style={{ padding: 4 }}>
            {RESPONSE_CODES.map(r => (
              <div key={r.code} style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 14, padding: '10px 14px', borderBottom: '1px solid var(--line)', alignItems: 'center' }}>
                <Status code={r.code} />
                <div>
...
function ModuleCard({ mod, onOpen }: { mod: typeof MODULES[0]; onOpen: () => void }) {
  const fields = (MAPPINGS_BY_MODULE[mod.id] || []).length;
  return (
    <div onClick={onOpen} className="clickable" style={{ padding: 16, background: 'var(--bg-1)', border: '1px solid var(--line)', borderRadius: 10, transition: 'all 0.15s', position: 'relative', overflow: 'hidden' }}
      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--line-warm)'; }}
      ...
      <div style={{ padding: 8, marginTop: 10, marginBottom: 10, background: 'var(--code-bg)', borderRadius: 6, border: '1px solid var(--line)', fontFamily: 'var(--font-jetbrains-mono), monospace', fontSize: 11.5, color: 'var(--ink-1)', wordBreak: 'break-all' }}>
