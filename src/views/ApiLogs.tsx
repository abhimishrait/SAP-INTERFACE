...
import { TRANSACTIONS, MODULE_BY_ID, MAPPINGS_BY_MODULE, SAMPLE_PAYLOADS, iso, getHeadersForTx, getResponseHeaders, AUTH_USER, RESPONSE_CODES } from '@/data';
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '22px 24px 12px' }}>
        <ViewHeader
          ...
          actions={
            <>
              <button className="btn" onClick={() => setStreaming(s => !s)}>{streaming ? <Icons.pause /> : <Icons.play />} {streaming ? 'Pause stream' : 'Resume'}</button>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1px 720px', overflow: 'hidden' }}>
        <div style={{ overflow: 'auto' }}>
          <table className="t">
            <thead><tr>
              <th>Time</th><th>Txn ID</th><th>Module</th><th>Endpoint</th><th>Status</th><th style={{ textAlign: 'right' }}>Duration</th>
            </tr></thead>
            <tbody>
              {filtered.map((t, i) => {
                ...
                return (
                  <tr key={t.id} onClick={() => setSelectedTxId(t.id)}
                    ...
                    <td style={{ fontSize: 11.5, fontWeight: 600 }}>
                      <span className="mono" style={{ fontSize: 10, color: 'var(--ink-3)', marginRight: 6 }}>{mod.code}</span>{mod.label}
                    </td>
        ...
        <TxInspector tx={selected || null} tab={rightPaneTab} setTab={setRightPaneTab} />
      </div>
    </div>
  );
}

function TxInspector({ tx, tab, setTab }: { tx: typeof TRANSACTIONS[0] | null; tab: string; setTab: (t: string) => void }) {
...
function TabRequest({ tx }: { tx: typeof TRANSACTIONS[0] }) {
...
function TabResponse({ tx }: { tx: typeof TRANSACTIONS[0] }) {
  const sample = SAMPLE_PAYLOADS[tx.moduleId];
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
...
function TabMapping({ tx }: { tx: typeof TRANSACTIONS[0] }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 10, border: '1px solid var(--line)', borderRadius: 8, overflow: 'hidden' }}>
        <table className="t">
          <thead><tr><th>SAP B1 field</th><th>Transform</th><th>DMS field</th><th>Result</th></tr></thead>
          <tbody>
            {rows.map((r, i) => (
              <tr key={i}>
...
function TabDbWrites({ tx }: { tx: typeof TRANSACTIONS[0] }) {
  const mod = MODULE_BY_ID[tx.moduleId];
  const moduleTable = ({
    'bp-master': 'bp_master',
  ...
  const txIdx = TRANSACTIONS.indexOf(tx);
  const writes = [
    ...
    { table: moduleTable, op: tx.method === 'POST' ? 'INSERT' : 'UPDATE', pk: tx.method === 'POST' ? `id=${4218 + txIdx}` : tx.customerCode, bytes: tx.bytesIn, ms: 5.4 },
...
function TabTimeline({ tx }: { tx: typeof TRANSACTIONS[0] }) {
  const mod = MODULE_BY_ID[tx.moduleId];
  const steps = [
